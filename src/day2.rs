...
fn test_calculate_score_by_choosing_shape() -> Result<()> {
	assert_eq!(
		get_score_from_strategy_guide(File::open("inputs/day2.txt")?, choose_shape_by_char)?,
		11906
	);
	Ok(())
}
...
fn test_calculate_score_by_choosing_outcome() -> Result<()> {
	assert_eq!(
		get_score_from_strategy_guide(File::open("inputs/day2.txt")?, choose_shape_by_outcome)?,
		11186
	);
	Ok(())
}
	...
	S: Fn(Shape, &char) -> Result<Shape>,
{
	...
	for (i, line) in BufReader::new(strategy_guide).lines().enumerate() {
		...
		let round_outcome: RoundOutcome = match player_shape {
			_ if enemy_shape == player_shape => RoundOutcome::Draw,
